...
if [ "$1" == 'pc' ]; then
    for width in "1024" "2048" "4096" "8192" "16384" "32768" "65536" "131072"; do
        for i in "1" "2" "4"; do
            execution ../gen_graphs/output/simple_circuit_n${width}_d${temp}_th${i}.json ${i}_stock "$compiler" "$last_bin_dir" "$width"
            python gen.py ${i} 1 0 ../gen_graphs/output/simple_circuit_n${width}_d${temp}_th${i}.json cut.json --mode 3 --shuffle_layers False --inside_layer_schedule backpack
            execution cut.json ${i}_depth "$compiler" "$last_bin_dir" "$width"
        done
    done
elif [ "$1" == 'server' ]; then
