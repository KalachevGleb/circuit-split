 ...
 */
// C++ implementation of MD5 (autogenerated from the above pseudocode)
...
vector<uint8_t> md5(const vector<uint8_t> &data) {
    // Initialize variables:
    ...
    for (int i = 0; i < 8; i++) {
        message.push_back((length >> (i * 8)) & 0xff);
    }
...
std::string md5AsHexString(const vector<uint8_t> &hash)
{
    std::string result;
...
std::string md5(const std::string &data)
{
    ...
    return md5AsHexString(md5(bytes));
}

string md5ForFile(const string &filename)
{
    vector<uint8_t> contents = readFileBinary(filename);
    return md5AsHexString(md5(contents));
}
